...
describe Admin do
  ...
  describe 'allows legitimate logins:' do
    ['123', '1234567890_234567890_234567890_234567890',
     'hello.-_there@funnychar.com'].each do |login_str|
      it "'#{login_str}'" do
        lambda do
          u = create_admin(:login => login_str)
  ...
  describe 'allows legitimate emails:' do
    ['foo@bar.com', 'foo@newskool-tld.museum', 'foo@twoletter-tld.de', 'foo@nonexistant-tld.qq',
     ...
     'domain@can.haz.many.sub.doma.in', 'student.name@university.edu'
      ...
      it "'#{email_str}'" do
        lambda do
          u = create_admin(:email => email_str)
  ...
  it 'does not rehash password' do
    ...
    Admin.authenticate('quentin2', 'monkey').should == admins(:quentin)
  end
  ...
  it 'authenticates admin' do
    Admin.authenticate('quentin', 'monkey').should == admins(:quentin)
  end

  it "doesn't authenticate admin with bad password" do
    Admin.authenticate('quentin', 'invalid_password').should be_nil
  end

 if REST_AUTH_SITE_KEY.blank?
   # old-school passwords
   it "authenticates a user against a hard-coded old-style password" do
     Admin.authenticate('old_password_holder', 'test').should == admins(:old_password_holder)
   end
 else
   it "doesn't authenticate a user against a hard-coded old-style password" do
     Admin.authenticate('old_password_holder', 'test').should be_nil
   end
   ...
   it "takes longer than #{desired_encryption_expensiveness_ms}ms to encrypt a password" do
     test_reps = 100
     start_time = Time.now; test_reps.times{ Admin.authenticate('quentin', 'monkey'+rand.to_s) }; end_time   = Time.now
